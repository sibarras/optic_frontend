...
pub fn Calculations(
    ...
    let get_brand_name = move |id| brands().iter().find(|s| s.id == id).unwrap().name.clone();
    let get_kind_name = move |id| kinds().iter().find(|s| s.id == id).unwrap().name.clone();
    ...
    let total_attended = move || sales().iter().map(|s| s.quantity).sum::<i32>();
